...
def triton_flash_attn_fn(
    ...
    if multiquery:
        # necessary to clone tensor because output contains NaN in edge cases
        # such as with head dimension = 8
        key = key.expand(*key.shape[:2], n_heads, key.size(-1)).clone()
        value = value.expand(*value.shape[:2], n_heads, value.size(-1)).clone()
