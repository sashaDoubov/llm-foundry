...
def train(cfg: DictConfig) -> Trainer:
    ...
    mono_class = callbacks.get('mono_checkpoint_saver_fixed')
    out = list(filter(lambda x: isinstance(x, mono_class), trainer.state.callbacks))
    assert len(out) == 1

    mono_chkpt_callback = out[0]

    mono_chkpt_callback._save_checkpoint(trainer.state, trainer.logger)
