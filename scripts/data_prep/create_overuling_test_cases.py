...
def main():
    get_tokenized_lengths = True

    if get_tokenized_lengths:
    ...
    for df, local_jsonl_path, upload_path in ([
            df_full.head(), local_jsonl_small_path, upload_path_small
    ], [df_full, local_jsonl_big_path, upload_path_big]):
        df_filtered = df.dropna()

        # need to be careful with order, here: yes -> 1 (is overruling)
        json_list = df_filtered.apply(lambda row: {
            'query': row['sentence1'],
            'choices': [
                row['no'],
                row['yes'],
        ...
        json_list[0]
        print(f'# of examples {len(json_list)}')
        json_df = pd.DataFrame(json_list)
        print(json_df.head())

        if get_tokenized_lengths:
            max_length = df_filtered['sentence1'].apply(
                lambda x: len(tokenizer.tokenize(x))).max()
            print(f'max tokenized length: {max_length}')
